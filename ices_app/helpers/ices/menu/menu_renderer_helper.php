...
class Menu_Renderer {
    ...
    public static function modal_menu_render($app,$modal){
        //<editor-fold defaultstate="collapsed">
        $modal->header_set(array('title'=>Lang::get('Menu'),'icon'=>'fa fa-cogs'));
        $modal->width_set('95%');
        self::menu_components_render($app, $modal,true);
        //</editor-fold>
    }

    public static function menu_render($app,$form,$data,$path,$method){
        //<editor-fold defaultstate="collapsed">
        $path = Menu_Engine::path_get();
        
        $id_prefix = Menu_Engine::$prefix_id;
        
        $id = $data['id'];
        self::menu_components_render($app, $form,false);
    ...
    public static function menu_components_render($app,$form,$is_modal){
        //<editor-fold defaultstate="collapsed">
        ...
        $components[$id_prefix.'_status'] = $form->input_select_add()
            ...
            ->input_select_set('id',$id_prefix.'_menu_status')
        ...
        if($is_modal){
            $param['detail_tab'] = '#modal_menu .modal-body';
            $param['view_url'] = '';
            $param['window_scroll'] = '#modal_menu';
        }

        $js = get_instance()->load->view(ICES_Engine::$app['app_base_dir'] . 'menu/menu_u_group_js', $param, TRUE);
        $app->js_set($js);
        
        $js = get_instance()->load->view(ICES_Engine::$app['app_base_dir'].'menu/menu_basic_function_js',$param,TRUE);
        $app->js_set($js);
        //</editor-fold>
    }

    public static function menu_status_log_render($app,$form,$data,$path){
        $config=array(
            'module_name'=>'menu',
    ...
    public static function u_group_log_render($app,$pane, $data){
        ...
        $q = '
            select distinct
                ...
                e.username,
                ug.app_name
            from menu_u_group_log eugl
                inner join u_group ug on eugl.u_group_id = ug.id
                inner join menu e on eugl.modid = e.id 
            where eugl.menu_id = '.$db->escape($data['id']).'
        ...
        $table->table_set('id','menu_u_group_log_table');
