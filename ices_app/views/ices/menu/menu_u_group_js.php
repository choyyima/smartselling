<script>
    var menu_u_group_methods = {
        load_u_group: function(iparam) {
            var lparent_pane = menu_parent_pane;
            var lprefix_id = menu_component_prefix_id;
            var lmethod = $(lparent_pane).find(lprefix_id + '_method').val();
            menu_tbl_u_group_method.reset();
            menu_tbl_u_group_method.head_generate();
            $.each(iparam.u_group, function(lidx, lrow) {
                menu_tbl_u_group_method.input_row_generate(lrow);
            });
            menu_tbl_u_group_method.input_row_generate({});
        }
    }

    var menu_u_group_bind_event = function() {
        var lparent_pane = menu_parent_pane;
        var lprefix_id = menu_component_prefix_id;
        ...
        menu_tbl_u_group_method.setting.func_new_row_validation = function(iopt) {
            var lresult = {success: 1, msg: []};
            var success = 1;
            var lrow = iopt.tr;
            <?php // --- validation select menu category---  ?>
        ...
        menu_tbl_u_group_method.setting.func_get_data_table = function() {
            var lparent_pane = menu_parent_pane;
            var lprefix_id = menu_component_prefix_id;
            var lresult = {u_group: []};
        ...
        menu_tbl_u_group_method.setting.func_row_bind_event = function(iopt) {
            var lparent_pane = menu_parent_pane;
            var lprefix_id = menu_component_prefix_id;
            var lrow = iopt.tr;
...
<?php // --- End Of Show and Hide phase ---                   ?>
        ...
        menu_tbl_u_group_method.setting.func_row_transform_comp_on_new_row = function(iopt) {
            var lrow = iopt.tr;
        ...
        menu_tbl_u_group_method.setting.func_row_data_assign = function(iopt) {
            var lparent_pane = menu_parent_pane;
            var lprefix_id = menu_component_prefix_id;
            var ldata_row = iopt.data_row;
            var lrow = iopt.tr;
            ...
            switch (lmethod) {
                ...
                case 'view':
                    if (Object.keys(ldata_row).length > 0) {
                        ...
                        menu_tbl_u_group_method.components.trash_set(iopt);
                    }
                    break;
            }
